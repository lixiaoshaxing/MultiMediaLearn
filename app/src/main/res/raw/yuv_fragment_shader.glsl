...
vec2 cPos(float t,float shiftx,float shifty){
    vec2 pos=vec2(uWidth*vTextureCoordinate.x,uHeight*(vTextureCoordinate-shifty));
...
vec4 calculateY(){
    //填充点对应图片的位置
    float posX=floor(uWidth*vTextureCoordinate.x);
    float posY=floor(uHeight*vTextureCoordinate.y);
    //实际采样起始点对应图片的位置
...
vec4 calculateU(){
    ...
    float posX=floor(uWidth*vTextureCoordinate.x);
    float posY=floor(uHeight*(vTextureCoordinate.y-0.2500));
    //实际采样起始点对应图片的位置
...
vec4 calculateV(){
    ...
    float posX=floor(uWidth*vTextureCoordinate.x);
    float posY=floor(uHeight*(vTextureCoordinate.y-0.3125));
    //实际采样起始点对应图片的位置
    float rPosX=mod(posX*8.,uWidth);
    float rPosY=posY*16.+floor(posX*8./uWidth)*2.;
